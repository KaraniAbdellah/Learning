...
/*
    JSON
    - JSON.parse => Conveffrt JSON To JS Object
    - JSON.stringify => Convert JS Object To JSON
*/
...
/*
    ...
    - Each Operation Must Wait For The Previous Operations To Complete
    - Story From Real Life

    Asynchronous
    - Operations Runs In Parallel
    - This Means That An Operation Can Run while Another One Is Still Being Processed
    - Story From Real Life
...
/*
    ...
    - Promise Use For Avoid Callback Hell
...
// print before check promise in pending
const myPromise1 = new Promise(function(resolveFun, rejecteFun) {
    // do some operations ... 
    let connect = false; // operations failed
    if (connect) {
        resolveFun("Connection Succefully");
    } else {
        rejecteFun("Conenction Failed");
    }
}).then(
    (resolveValue) => console.log(`Good ${resolveValue}`),
    (rejectValue) => console.log(`Bad ${rejectValue}`), // Conenction Failed
);
console.log(myPromise1);

// print after check promise
const myPromise2 = new Promise(function(resolveFun, rejecteFun) {
    let connect = false;
    if (connect) {
        resolveFun("Connection Succefully");
    } else {
...
/*
    ...
    let Employees = []; // Each then We Handle An Error
    If then Chained --> Just One Handle Error
*/


const myPromise = new Promise((resolveFun, rejecteFun) => {
    // let Employees = ["abdellah", "Ahmed", "Omaima", "Khadija"];
    let Employees = [];
    if (Employees.length == 4) {
        resolveFun(Employees);
    } else {
        rejecteFun(Error("Number Of Employees Is Not 4"));
    }
});

myPromise.then(resolveValue => {
    resolveValue.length = 2;
    console.log(resolveValue); // ['abdellah', 'Ahmed']
    return resolveValue;
}).then(resolveValue => {
    ...
    console.log(`The Choosen Employee is ${resolveValue}`); // The Choosen Employee is abdellah
}).catch(rejected =>{
    console.log(`rejected : ${rejected}`); // rejected : Error: Number Of Employees Is Not 4
}).finally(message => console.log("done!")); // done!





// Promise Search [all && race]
// Example 1
